...
st.markdown("<h1 style='text-align: center; color: blue;'>Singapore Flat Resale Price Predicton</h1>",
            unsafe_allow_html=True)

df = pd.read_csv(r'C:\Users\Avinash Bala\Desktop\DATA SCIENCE\project\singapore resale flat project\final.csv')
...
with tab1:
    ...
    with col2:
        ...
        st.write(
            '* Comparing other regressors, Random Forest Regressor had a high :red[R-squared score], which means it has performed best. ')
    ...
    if option == 'Prediction Tab':
        ...
        with col3:
            town_mapping = {'ANG MO KIO': 1, 'BEDOK': 2, 'BISHAN': 3, 'BUKIT BATOK': 4, 'BUKIT MERAH': 5,
                            ...
                            'QUEENSTOWN': 16,
                            'SENGKANG': 17, 'SERANGOON': 18, 'TAMPINES': 19, 'TOA PAYOH': 20, 'WOODLANDS': 21,
                            'YISHUN': 22,
...
with tab3:
    ...
    if st.button('Predict and Conclusion'):
        def load_data():
            # Load data
            data = pd.read_csv(r'C:\Users\Avinash Bala\Desktop\DATA SCIENCE\project\singapore resale flat project\final.csv')
        ...
        st.write(
            '* Resale price of the flats are majorly depends on the number of :blue[storeys in the flat, Flat type, Floor area].')
        st.write(
            '* Studying historical resale data and identifying market trends can provide insights into potential future price movements. ')
        st.write(
            '* Proximity to amenities such as :orange[MRT stations, schools, shopping malls, parks], and other facilities can significantly impact resale prices.')
        st.write(
            '* Older flats may have lower resale values, especially if they require renovations or if the lease is running out.')
        st.write(
            '* :red[Population growth, changes in demographics], and :red[urban development plans] can influence the demand for housing in certain areas.')
